...
// 2e. Parent/Student sees 0 cards
  ...
  switch (route) {
    ...
    case "/reports/students":
      if (
        ...
        return { allowed: false, reason: "Student list report requires academic reporting permissions" };
      }
      return { allowed: true };
...
// 3b. Teacher workload direct route check
assert.equal(checkDirectRouteAccess("/reports/teachers", superAdminAcl).allowed, true);
assert.equal(checkDirectRouteAccess("/reports/teachers", academicAdminAcl).allowed, true);
assert.equal(checkDirectRouteAccess("/reports/teachers", teacherAcl).allowed, false);
assert.equal(
  checkDirectRouteAccess("/reports/teachers", teacherAcl).reason,
  "Teacher workload report requires school scope"
);

// 3c. Unprivileged user blocked from all routes
