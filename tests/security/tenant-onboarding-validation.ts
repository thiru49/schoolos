...
function contains(hay: string, term: string) {
  ...
  if (hay.toLowerCase().includes(hex.toLowerCase().slice(0, 16))) return true;
...
async function main() {
  ...
  try {
    ...
    // ── 5. Receipt PDF ───────────────────────────────────────────────────────
    const studentsB = (await (await authed(adminB.accessToken, "/students")).json()) as {
      id: string;
      fullName: string;
    }[];
    ...
    const studentsA = (await (await authed(accountsA.accessToken, "/students")).json()) as {
      id: string;
      fullName: string;
    }[];
    const arun = studentsA.find((s) => s.fullName.startsWith("Arun"));
    ...
    // ── 6. Report card PDF ───────────────────────────────────────────────────
    const sectionsB = (await (await authed(adminB.accessToken, "/academics/sections")).json()) as {
      id: string;
      classId: string;
      label: string;
    }[];
    const sectionB = sectionsB[0];
    ...
    record(
      ...
      restored.schoolName === originalB.name && restored.receiptPrefix === originalB.receiptPrefix,
      restored.schoolName,
    );
  }
